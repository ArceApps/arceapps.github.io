---
title: "Automated QA as an Indie: Testing the ArceApps Ecosystem (ArceApps)"
...
lastmod: "2024-05-19"
author: "ArceApps"
keywords: ["ArceApps", "devlog", "testing", "qa", "playwright"]
canonical: "https://arceapps.com/devlog/automated-qa-indie-2024/"
...
test('verify devlog article renders and images load', async ({ page }) => {
  ...
  await page.goto('http://localhost:4321/devlog/refining-experience');

  await expect(page).toHaveURL(/\/devlog\/refining-experience/);
  await expect(page).toHaveTitle(/Refining the Experience/);
...
This Playwright spec validates the canonical devlog route directly. The script previously verified that legacy `/mi-historia/*` routes redirected to the new `/devlog/*` architecture; those redirects were removed in July 2026 once the URL change had been consolidated for over a year in production. The test now navigates directly to the canonical URL and verifies that both content and SVG hero images render correctly. The explicit evaluation of natural width (`naturalWidth`) guarantees that the images are not just present in the DOM, but have successfully rendered over the network.

...
- [ ] **Task 2: Perform Visual Verification and Compile**
  - [ ] Step 1: Start dev server with `pnpm dev` and preview the table inside the blog post "AI Tools Worth Learning in 2026" in dark mode.
  - [ ] Step 2: Audit other key components and layouts (Footer, Header, PhilosophyCard, ContactForm) in dark mode to ensure no other text has poor contrast.
  - [ ] Step 3: Create the final verification report [dark-mode-tables-verify.md](file:///home/arceappspc/Projects/ArceApps/arceapps.github.io/docs/specai/dark-mode-tables/dark-mode-tables-verify.md).
  - [ ] Step 4: Build project in production mode with `pnpm build` to confirm output validity.
  - [ ] Step 5: Commit the verification report with `git commit -m "docs: add dark mode verification report"`.
